...
$pages = array(
    ...
    'how_it_works' => array( 'title' => 'How it works', 'slug' => 'how-it-works', 'template' => 'page-templates/template-how-it-works.php', 'content' => '<h2>Send a rental request in clear steps</h2><ol><li>Choose your preferred vehicle from the fleet.</li><li>Add your pickup and return dates, times and airport locations.</li><li>For airport pickup, provide your flight number so we can follow your arrival.</li><li>Choose insurance and any optional extras you need.</li><li>Send the request without making a payment.</li><li>Our team checks the selected vehicle and confirms availability, the final price and rental conditions personally.</li><li>Confirm directly with our team before collection.</li></ol><p><strong>Submitting this request does not immediately confirm the reservation. We will check availability and contact you.</strong></p><p><a href="/fleet/">Browse the fleet</a> or read the <a href="/rental-requirements/">rental requirements</a> before sending a request.</p>' ),
    'rental_requirements' => array( 'title' => 'Rental requirements', 'slug' => 'rental-requirements', 'template' => 'page-templates/template-rental-requirements.php', 'content' => '<h2>Driver and document requirements</h2><p>Drivers must be at least 23, hold an original valid category B licence for at least three years, and present a valid passport or national identity card. The licence must be legally accepted for driving in Italy. Non-EU/EEA licences may require an international driving permit or an official Italian translation. Additional drivers must meet the same requirements.</p><h2>Payment, deposit and mileage</h2><p>No payment is required to send a request. Rental payment and the deposit are paid at pickup; credit and debit cards are accepted. Displayed rental prices include VAT and RCA. The deposit is €350 for vehicles up to five seats and €500 for vehicles with seven to nine seats. It is separate from the rental total and is released after the return inspection when no charges remain; bank processing times may vary.</p><p>Each rental day includes 150 km. Additional kilometres cost €0.10 each. Return the vehicle with the same fuel level supplied; missing fuel may be charged.</p><h2>Pickup, use and return</h2><p>For an airport pickup, provide a valid flight number and contact us if your arrival changes. After-hours pickup, insurance options, child seats and additional drivers are confirmed in the request and rental contract. Only authorised drivers may use the vehicle. Observe traffic, parking, toll and ZTL rules; cross-border travel requires prior authorisation. Report accidents, theft, breakdowns or expected delays immediately and do not arrange repairs without authorisation.</p><p>Free cancellation is available up to 24 hours before pickup. Changes remain subject to availability and a price adjustment. Read the <a href="/terms-and-conditions/">Terms and Conditions</a> for the complete rental terms.</p>' ),
    ...
    foreach ( $pages as $key => $page ) {
        ...
        foreach ( array( 'en', 'ro', 'ru' ) as $language ) {
            ...
            if ( ! empty( $translations[ $language ] ) ) {
                ...
                if ( $apply && $is_provisioned_content ) {
